...
namespace
{
    ...
    GLuint gPlaneProgramId; //plane/desk
    GLuint gSodaProgramId;  //soda
    GLuint gCubeProgramId;  //ipad but will be cube
    GLuint gCube2ProgramId; //soda tab
...
int main(int argc, char* argv[])
{
    ...
    if (!UCreateShaderProgram(cubeVertexShaderSource, cubeFragmentShaderSource, gSodaProgramId))  //soda
        return EXIT_FAILURE;
    ...
    // Load texture
    const char* texFilename = "toy.jpg";   //toy
    if (!UCreateTexture(texFilename, gTextureId))
    {
        cout << "Failed to load texture " << texFilename << endl;
        return EXIT_FAILURE;
    }
    texFilename = "galaxy.png";              //plane
    if (!UCreateTexture(texFilename, gPlanePattern))
    {
        cout << "Failed to load texture " << texFilename << endl;
        return EXIT_FAILURE;
    }
    texFilename = "aluminum1.jpg";          //soda can
